...
class RedirectStderrFD:
    # Capture OS-level stderr
    def __enter__(self):
        import tempfile, os
        self._os = os
        self._tmp = tempfile.TemporaryFile(mode='w+b')
        self._tmp_fd = self._tmp.fileno()
        self._saved_fd2 = os.dup(2)      # duplicate current fd=2
        os.dup2(self._tmp_fd, 2)         # redirect fd=2 to temp file
        return self
    def __exit__(self, exc_type, exc, tb):
        os = self._os
        try:
            os.dup2(self._saved_fd2, 2)  # restore original fd=2
        finally:
            os.close(self._saved_fd2)
        self._tmp.seek(0)
        try:
            self.captured = self._tmp.read().decode('utf-8', errors='replace')
        finally:
            self._tmp.close()

PRINT_HEADER_IN_WORKER = False # only parent prints banner, not workers
...
def process_one_pdb(pdbfile, solved_struct_path, rmsd_threshold, query_pdbs_dir, 
    ...
    if PRINT_HEADER_IN_WORKER:
        print('\n' + '#' * 30, pdbfile, '#' * 30)

    _stdout_buf = io.StringIO()
    _stderr_buf = io.StringIO()

    # Place RDKit control inside the try so any rdkit errors are caught
    try:  # Prevent non-picklable exceptions escaping the worker
        try:
            RDLogger.DisableLog('rdApp.info')
        except Exception:
            pass
        # FD-level stderr capture (for rdkit c++ logs) + python stdout/stderr
        with RedirectStderrFD() as _fdcap:
        ...
        _captured_rdkit = getattr(_fdcap, 'captured', '')
        return (pdbfile, failed_vdg_files, _stdout_buf.getvalue(), _stderr_buf.getvalue(), _captured_rdkit)
        ...
        try:
            ...
            _captured_rdkit = ''
        return (pdbfile, [], _stdout_buf.getvalue(), _stderr_buf.getvalue(), _captured_rdkit)
...
def main():
    ...
    _solved_struct_for_header = pr.parsePDB(solved_struct_path)
    _lig_obj = _solved_struct_for_header.hetatm.select(
        'not (ion or resname SEP or resname TPO or resname MSE)')
    if _lig_obj is None:
        raise ValueError(
            f'Prody could not find ligand atoms in {solved_struct_path}. You may need to '
            f'reformat the lig resname in the PDB file.')
    _ligname = list(set(_lig_obj.getResnames()))[0]
    # Call once with quiet=False to emit the pymol selection header
    dock.get_bindingsite_residues(
        _solved_struct_for_header, [], _ligname,
        dist_from_lig=4.5, CA_only=False, quiet=False)
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        try:
            for fut in concurrent.futures.as_completed(futures):
                ...
                try:
                    # Unpack captured logs
                    result = fut.result()
                    if not (isinstance(result, tuple) and len(result) == 5):
                        # Group the error under the PDB banner
                        banner = '\n' + '#' * 30 + f' {pdbfile} ' + '#' * 30 + '\n'
                        sys.stdout.write(banner)
                        print(f"ERROR: {pdbfile} failed: Unexpected worker return value")
                        continue

                    pdbfile, failed, captured_out, captured_err, captured_rdkit = result
                    failed_vdg_files_global.extend(failed)

                    # Print one coherent block per PDB under its banner
                    banner = '\n' + '#' * 30 + f' {pdbfile} ' + '#' * 30 + '\n'
                    sys.stdout.write(banner)
                    if captured_out:
                    ...
                    if captured_err:
                        ...
                        if not captured_err.endswith("\n"):
                            sys.stdout.write("\n")
                    if captured_rdkit:
                        sys.stdout.write(captured_rdkit)  # includes WARNING/ERROR/CRITICAL
                        if not captured_rdkit.endswith("\n"):
                            sys.stdout.write("\n")

                except Exception as e:
                    # Group the error beneath a banner for this PDB so it doesn't float elsewhere
                    banner = '\n' + '#' * 30 + f' {pdbfile} ' + '#' * 30 + '\n'
                    sys.stdout.write(banner)
