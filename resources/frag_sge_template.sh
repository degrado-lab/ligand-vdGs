...
$PRE_RUN
# Absolute path, substituted at generation time. 
python $WRAPPER -s $SMILES -c $CG -p $PDB_DIR -o $OUTPUT_DIR -m $MAX_NUM_CLUS --subset-sizes $SUBSET_SIZES --num-procs $NUM_PROCS
